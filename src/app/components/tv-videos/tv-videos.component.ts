...
@Component({
  ...
  styleUrl: './tv-videos.component.scss'
})
export class TvVideosComponent implements OnInit, OnDestroy{
    ...
    this._ActivatedRoute.paramMap.subscribe({
      next:(params)=>{
        ...
        this._DetailsService.getDetails('tv',this.mediaId).subscribe({
          next:(res)=>{
            this.TVDetails = res;
            this._TitleService.setTitle(this.TVDetails.name + ' - Videos');
          }
        });
        this._DetailsService.getVideos('tv', this.mediaId).subscribe({
          next: (res) => {
            this.TVVideos = res.results;
            const groupedVideos = this.TVVideos.reduce((finalArray: any, video: any) => {
            ...
            this.TVTypedVideos = groupedVideos;
            ...
            this.videoKeys.forEach((type) => {
              ...
              this.TVTypedVideos[type].forEach((video: any, index: number) => {
